...
public class WaterJarDFS {
    ...
    static void dfs(int capacity1,int capacity2,int target,Stack<JugState> stack,int[][] visited)
    {
      JugState start = new JugState(0,0,null);
      stack.push(start);// appending start state in the Queue
      while(!stack.isEmpty())
      {
        JugState current = stack.pop();// pop the topmost element
    ...
    public static void main(String[] args)
    {
      ...
      Stack<JugState> stack = new Stack<>();// queue for breadth first search since we are exploring all nodes at a particular depth
      dfs(capacity1,capacity2,target,stack,visited);
  }
}
class JugState
{
