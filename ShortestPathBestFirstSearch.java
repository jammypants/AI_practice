...
public class ShortestPathBestFirstSearch {
	...
	public static int bestfirstsearch(int adjMatrix[][],int s,int d,HashMap<Integer,Integer> map) {
		...
		while(!q.isEmpty()) {
			int size = q.size();
				...
				if(curridx==d) {
       	     	...
       	     	set.add(curridx);
				for(int j=0;j<adjMatrix.length;j++)// append all adjacent routes to the priority queue
				 {
	...
	public static void main(String[] args) {
		...
		map.put(6, 98);
		map.put(7, 0);
		int cost = bestfirstsearch(adjMatrix,0,7,map);
		System.out.println();
